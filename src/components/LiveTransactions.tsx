...
// Ensure public RPC configurations use a secure fallback setup
const DRPC_WSS = process.env.NEXT_PUBLIC_POLYGON_WSS || "wss://lb.drpc.live/og/polygon/fallback";

interface UIEventTransaction {
  id: string;
  fullHash: string;
  amount: string;
  time: string;
...
export default function LiveTransactions() {
  const [transactions, setTransactions] = useState<UIEventTransaction[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      provider = new ethers.WebSocketProvider(DRPC_WSS);
      contract = new ethers.Contract(HABA_ADDRESS, HABA_ABI, provider);
      ...
      contract.on("Transfer", (from: string, to: string, value: any, event: any) => {
        ...
        // Securely parse string coordinates cleanly to prevent template extraction vulnerabilities
        const truncatedId = `${txHash.substring(0, 6)}...${txHash.substring(txHash.length - 4)}`;
        
        let formattedAmount = "0";
        try {
          formattedAmount = parseFloat(ethers.formatUnits(value, 18)).toLocaleString(undefined, {
            maximumFractionDigits: 4
          });
        } catch (e) {
          console.error("Token translation conversion overflow rejected.");
        }

        const newTx: UIEventTransaction = {
          id: truncatedId,
          fullHash: txHash.replace(/[^a-fA-F0-0x]/g, ""), // Sanitize characters strictly
      ...
      console.error("WebSocket connection protocol initialization aborted safely: ", err);
    }
  ...
  return (
    <div className="bg-black/40 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-6 shadow-2xl select-none">
          ...
          transactions.map((tx) => (
            <div key={tx.fullHash} className="flex justify-between items-center p-3 bg-white/[0.02] hover:bg-white/[0.05] rounded-2xl transition-all group border border-transparent hover:border-white/10">
              ...
              <div className="flex items-center gap-4">
                ...
                <a 
                  href={`https://polygonscan.com/tx/${encodeURIComponent(tx.fullHash)}`} 
                  target="_blank" 
