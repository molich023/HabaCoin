...
export default function GenesisCertificate({ userName, habaBalance }: GenesisCertificateProps) {
  const { toPDF, targetRef } = usePDF({ filename: `Haba_Genesis_Pioneer.pdf` });

  const safeBalance = typeof habaBalance === 'number' ? habaBalance : 0;
  const safeName = typeof userName === 'string' ? userName.replace(/[<>]/g, "") : "Pioneer Node";

  if (safeBalance < 1000) return null;

  const generateSecureHash = (input: string): string => {
    try {
      // Basic client-side obfuscation. True validation hashes must match your ledger block heights.
      return btoa(encodeURIComponent(input)).substring(0, 12).toUpperCase();
    } catch {
      return "NODE-FALLBACK-ERR";
    }
  };
        ...
        >
          ...
          <div className="flex justify-between items-end mt-12">
            <div className="text-left">
              ...
              <p className="text-[8px] text-gray-500 font-mono">HABA-2026-{generateSecureHash(safeName)}</p>
            </div>
